...
test04()
{
  ...
  // std::cout << "copycounter: " << copycounter::copycount << std::endl;
  VERIFY(copycounter::copycount == 20);
  a.insert(a.end(), 50, c);
  VERIFY(copycounter::copycount == 70);
...
test05()
{
  ...
  a.reserve(1000);
  a.insert(a.begin(), 20, c);
  //Note: the libstdc++ implementation will make a temporary copy when no reallocation happen,
  // but our implementation doesn't, so i comment the following line.
